...
const Scan = () => {
  ...
  const fetchUserData = async () => {
    ...
    try {
      const response = await axios.get(
        `http://192.168.180.140:33000/api/users/email/${email}`
      );
  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    try {
      const response = await axios.get(
        `http://192.168.180.140:33000/api/users/${scannedUserId}`
      );
  ...
  const handleHealthReport = async () => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.180.140:33000/api/labReportShares",
  ...
  const fileUpload = async () => {
    ...
    try {
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        const response = await axios.post(
          "http://192.168.180.140:33000/api/shareFiles",
          {
